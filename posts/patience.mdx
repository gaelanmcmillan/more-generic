...
### Nesting Intervals
Alright, nesting intervals! Our work is cut out for us. We should quickly consider the naive case: comparing every sequence of intervals to every other sequence. This would be *O(n!)*—far too slow for any considerable set of intervals. If brute force is off the table, perhaps we can be more clever. Let's sort the intervals from largest to smallest and list them from top to bottom!
