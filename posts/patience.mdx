...
{/*
0. Introduce the problem
1. Build a mathematical model
2. Generic solution
3. Optimize solution for special case
*/}

## 0. The Problem
In early 2021, while working through the [Kattis archives](), I came across a tricky problem.
The problem, named simply *Poklon* (Croatian for *gift*), tells a brief, nonsensical story about a boy named Mirko who just received a set of intervals for his Birthday. An interval is a pair of numbers *\{start, end\}* with *start \<= end*.

One of the things Mirko likes to do with his intervals is "nest" them. An interval *b* can nest inside another interval *a* if *b*'s start point is greater than or equal to that of *a*, and *b*'s end point is less than or equal to that of *a*. If these conditions are met, we say *a* contains *b*, or *b* is contained by *a*.
...
### Nesting Intervals
Alright, nesting intervals! Our work is cut out for us. We should quickly consider the naive case: comparing every sequence of intervals to every other sequence.
This would be *O(n!)*—far too slow for any considerable set of intervals. If brute force is off the table, perhaps we can be more clever. 
Let's sort the intervals from largest to smallest and list them from top to bottom!
...
You are given a deck of *N* cards, with each card numbered from 1 to *N*. The deck is shuffled and placed face down. The game is played by drawing cards from the top of the deck and placing them face up in piles. A card can be placed on top of any card with a higher face value. The goal of the game is to turn each card in the deck while making as few piles as possible.
...
Order: a < b if (ai < bi and ai > bi)

As an unordered set, elements are now only comparable if their indices increase
