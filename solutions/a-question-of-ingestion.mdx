...
def score(course, consecutive_courses):
    ...
    if (dp[course][consecutive_courses] != -1):
        return dp[course][k]
    return score(c+1, m+1, depth+1) + min(hunger(consecutive_courses), cals_per_course[course])

for c in range(courses-1, -1, -1):
    for k in range(max_consecutive_courses-1, -1, -1):
        take  = score(course=c,   consecutive_courses=m)
        skip1 = score(course=c+1, consecutive_courses=max(0,m-1))
        skip2 = score(course=c+2, consecutive_courses=0)
        dp[c][m] = max(take, skip1, skip2)
